...
const CarHeader({super.key, required this.car, this.disableKmUpdate = false, required bool hideVehicleInfo});
  ...
  Widget build(BuildContext context) {
    ...
    // 2. جلب البيانات من الـ MaintenanceProvider
            ...
            child: Row(
              children: [
                ...
                _buildCompactInfo(
                  ...
                  '${car.mileageKm}',
                  AppColors.primary,
               onTap: disableKmUpdate ? null : () => _showUpdateKmDialog(context, appProvider, mainProvider),
                ),
                _divider(),
          ...
          CupertinoDialogAction(
            ...
            onPressed: () async {
              ...
              if (newKm != null) {
                ...
                // إعادة مزامنة الصيانة بناءً على الرقم الجديد
                if (success) {
                  await mainProvider.fetchDueMaintenance(
                    car.id,
                    newKm,
                  ); // ✅ ضيف الـ newKm كمعامل تاني
                }
              }
            },
          ),
